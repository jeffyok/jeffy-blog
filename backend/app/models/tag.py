...
article_tags = Table(
    "article_tags",
    Base.metadata,
    Column("article_id", ForeignKey("articles.id", ondelete="CASCADE"), primary_key=True, comment="文章ID"),
    Column("tag_id", ForeignKey("tags.id", ondelete="CASCADE"), primary_key=True, comment="标签ID"),
    comment="文章与标签关联表",
)
